...
def test_scheduler_overhead_accumulates(tmp_path: Path):
    ...
    assert first >= 0.04  # slack for scheduler jitter
...
def test_examples_per_second_ema_smooths(tmp_path: Path):
    ...
    # slower step: EMA should move down but not all the way to the latest rate
    logger.step_start()
    time.sleep(0.05)
    logger.step_end(num_examples=32)
    smoothed = logger.examples_per_second

    assert smoothed < fast
...
@pytest.mark.skipif(
    ...
    reason="CPU-only check; on CUDA peak memory is real",
)
