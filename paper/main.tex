...
\begin{abstract}
Parameter-efficient fine-tuning methods such as LoRA have become a
standard tool for adapting large language models under tight memory and
compute budgets. However, the standard formulation of LoRA assigns the
same rank to every adapted layer, despite the fact that a single unit
of rank costs a different number of trainable parameters at each
module. This work studies whether folding an explicit per-rank
parameter cost into the rank allocation rule yields a more efficient
distribution of a fixed total budget. Specifically, we propose a
lightweight two-stage allocator that first trains a uniform-rank model
during a brief warmup phase, then assigns ranks across layers a single
time using a score $s_i = g_i / c_i^{\alpha}$ --- where $g_i$ tracks
an exponential moving average of the LoRA gradient norms at module $i$
and $c_i$ is the per-rank parameter cost at the same module. As opposed
to more elaborate adaptive schemes like AdaLoRA, this approach is
straightforward to implement and runs its allocation step only once
per training run. We compare the proposed method against Uniform LoRA,
AdaLoRA, and a gradient-only ablation ($\alpha{=}0$) on SST-2 with
DistilBERT, reporting validation accuracy and loss, time to a $0.90$
accuracy target, peak GPU memory, training throughput, and scheduler
overhead. Under a fixed $192$-rank budget, the hardware-aware variant
($\alpha{=}1$) achieves the lowest trainable-parameter count of any
method considered ($1{,}081{,}346$ vs.\ AdaLoRA's $1{,}366{,}562$),
comparable validation accuracy with the lowest seed-to-seed variance
($\pm 0.001$ over three seeds), and roughly a $19\times$ reduction in
scheduler overhead compared with AdaLoRA ($4.0$\,s vs.\ $76.7$\,s).
An $\alpha$-sweep across $0.0$, $0.5$, and $1.0$ further reveals the
share of rank assigned to attention modules climbing from $39.1\%$ to
$56.1\%$ while the gradient signal is held fixed, suggesting that the
cost term genuinely reshapes the allocation rather than merely tracking
gradient magnitude. The picture against Uniform LoRA is somewhat more
mixed --- peak memory and time-to-target favor Uniform on this task
--- which we report as observed rather than retune.
\end{abstract}
...
as our ablation baseline; $\alpha = 1$ is the headline hardware-aware
variant we study throughout the paper.

We evaluate the resulting allocator on SST-2 sentiment
...
\item a systems-level evaluation that reports peak GPU memory,
      ...
      \emph{scheduler overhead} --- the last of which surfaces as our
      clearest systems-level finding (\S\ref{sec:results-systems});
...
clearest systems-level finding in our evaluation. Hardware-Aware's reallocator
