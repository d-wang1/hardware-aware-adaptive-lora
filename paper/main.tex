...
The continued scaling of transformer-based language models has driven
considerable interest in parameter-efficient fine-tuning, of which
Low-Rank Adaptation (LoRA) \cite{hu2022lora} is one of the most widely
adopted methods. LoRA freezes the pretrained weights and inserts
trainable rank-$r$ adapters of the form $\Delta W = BA$ into selected
linear layers, drastically reducing the number of parameters that must
be updated during fine-tuning. However, once the total rank budget
$B = \sum_i r_i$ has been fixed, an additional design question
surfaces: how should that budget be distributed among the adapted
modules? The standard formulation of LoRA answers this by simply
setting every $r_i$ to the same value. AdaLoRA
\cite{zhang2023adalora} answers it dynamically, redistributing rank
throughout training by pruning the SVD components that contribute the
least to the loss. Both approaches treat rank as if it were fungible
across the model --- that is, as if a single rank applied at one
module costs the same as a single rank applied at any other.

This treatment is, in practice, an oversimplification. On
...
trainable parameters as adding the same rank to a $768 \to 768$
attention projection. When the underlying constraint is parameter
count, GPU memory, or some closely related compute cost, charging
both modules the same ``rank price'' is a clear modeling shortcut. It
is therefore worth asking whether a rank allocator that explicitly
incorporates this cost asymmetry can produce more efficient
configurations under a constrained training budget.

This paper studies a small but explicit change to the LoRA rank
allocation rule: scale the gradient-based importance signal by the
inverse of the per-rank parameter cost. For each LoRA target module $i$,
\begin{equation}
...
proportion to $s_i$ while the total budget is preserved. Setting
$\alpha = 0$ collapses the rule to a gradient-only allocator and serves
as the ablation baseline; $\alpha = 1$ is the headline hardware-aware
variant studied throughout this paper.

I evaluate the resulting allocator on SST-2 sentiment
classification \cite{socher2013sst, wang2018glue} using DistilBERT,
comparing against Uniform LoRA, AdaLoRA \cite{zhang2023adalora} (via
the PEFT implementation~\cite{mangrulkar2022peft}), and the
$\alpha = 0$ gradient-only ablation. The contributions of this work
are threefold:
\begin{itemize}
\item the allocation rule in Eq.~\ref{eq:rule}, together with a
...
\item a systems-level evaluation that reports peak GPU memory,
      training throughput, total wall-clock, time-to-target, and
      \emph{scheduler overhead} --- the last of which surfaces as
      the clearest systems-level finding of this evaluation
      (\S\ref{sec:results-systems});
\item an $\alpha$-sweep that isolates the cost term, demonstrating
      that it monotonically reshapes the share of rank assigned to
      attention versus feed-forward modules while final accuracy and
      the upstream gradient signal are both held essentially fixed
      (\S\ref{sec:results-alpha}).
\end{itemize}
This paper makes no claim of universally outperforming AdaLoRA in
final accuracy. Rather, the goal is to characterize the practical
efficiency tradeoffs that emerge once a hardware-cost term is
introduced into an otherwise simple allocator, and to identify the
conditions under which the proposed rule does, or does not, deliver
useful systems-level improvements.
